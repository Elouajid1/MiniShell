...
{
	t_shell *shell = get_shell_instance(NULL);
	(void)signo;

	if (shell && shell->inside_prompt)
	{
...
// 		command = NULL;
// 		shell->token = NULL;
// 		shell->inside_prompt = 1;
// 		command = readline("minishell$ ");
// 		shell->inside_prompt = 0;
// 		if (!command)
// 			break ;
...
int	main_loop(t_shell *shell, char **env)
{
	...
	(void)env;
	get_shell_instance(shell); // register shell pointer
	// setup_signals();
	shell->interactive = isatty(STDIN_FILENO);
	while (1)
	{
