...
char *get_executable_path(char *cmd, t_shell *shell)
{
	...
	if (ft_strchr(cmd, '/'))
	{
		...
		{
			ft_putstr_fd("minishell: ", ST)
			printf("minishell: %s: No such file ore directory\n");
			shell->flag = 1;
			return (NULL);
		}
	}
	...
	if (ft_strchr(cmd, '/')) // Use your ft_strchr implementation
	{
		// It's a path, check if it exists and is accessible
		...
		if (access(cmd, X_OK) == -1)
		{
			...
			shell->flag = 4; // Permission denied
			return (NULL);
		}
		
		// All checks passed, return a copy of the path
		full_path = ft_strdup(cmd); // Use your ft_strdup
		if (!full_path)
		{
	...
	// If not found in PATH, it's a command not found error
	if (!full_path)
	{
		write(2, cmd, ft_strlen(cmd));
		write(2, ": command not found\n", 20);
		shell->flag = 2; // Command not found
		return (NULL);
	}
...
// Updated execute_single_command to handle all error flags
