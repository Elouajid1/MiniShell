...
static char	*handle_path_command(char *cmd, t_shell *shell)
{
	char	*full_path;
...
static char	*handle_dot_command(char *cmd, t_shell *shell)
{
	int	dots;
...
char	*get_executable_path(char *cmd, t_shell *shell)
{
	...
	if (ft_strchr(cmd, '/'))
		return (handle_path_command(cmd, shell));
	full_path = handle_dot_command(cmd, shell);
	return (full_path);
}
