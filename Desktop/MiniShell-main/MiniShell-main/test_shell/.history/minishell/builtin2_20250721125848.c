...
int	builtin_echo(char **argv)
{
	...
	newline = 1;  // By default, print newline
	i = 1;
	
	// Check for -n flag (can be -n, -nn, -nnn, etc.)
	if (argv[1] && argv[1][0] == '-' && argv[1][1] == 'n')
	{
		int j = 1;
		// Check that all characters after '-' are 'n'
		while (argv[1][j] && argv[1][j] == 'n')
			j++;
		
		// If we reached end of string and found only 'n' characters
		if (argv[1][j] == '\0' && j > 1)
		{
			newline = 0;
			i = 2;
	...
	// Print all arguments starting from index i
	while (argv[i])
	{
		ft_putstr_fd(argv[i], STDOUT_FILENO);
		if (argv[i + 1])  // Add space between arguments (not after the last one)
			ft_putchar_fd(' ', STDOUT_FILENO);
		i++;
	}
